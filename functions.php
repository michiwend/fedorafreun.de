...
load_theme_textdomain( 'nilmini', TEMPLATEPATH . '/languages' );
...
function nilmini() {
	...
	// Add a filter to nilmini_header_image_width and nilmini_header_image_height to change these values.
	define( 'HEADER_IMAGE_WIDTH', apply_filters( 'nilmini_header_image_width', 898 ) );
	define( 'HEADER_IMAGE_HEIGHT', apply_filters( 'nilmini_header_image_height', 290 ) );

	// Don't support text inside the header image.
	define( 'NO_HEADER_TEXT', true );

	// Add a way for the custom header to be styled in the admin panel that controls
	// custom headers. See nilmini_admin_header_style(), below.
	add_custom_image_header( '', 'nilmini_admin_header_style' );
...
function nilmini_search_form( $form ) {
    ...
    $form = '	<form method="get" id="searchform" action="'.get_bloginfo('url').'">
...
class nilmini_flickr extends WP_Widget {
	...
	function nilmini_flickr() {
		$widget_ops = array('description' => 'Show preview images from a flickr account or group.' , 'nilmini');

		parent::WP_Widget(false, __('Flickr Widget', 'nilmini'),$widget_ops);
	}
	...
	function widget($args, $instance) {  
        ...
        <div class="flickr_badge_wrapper"><script type="text/javascript" src="http://www.flickr.com/badge_code_v2.gne?count=<?php echo $number; ?>&amp;display=<?php echo $sorting; ?>&amp;&amp;source=<?php echo $type; ?>&amp;<?php echo $type; ?>=<?php echo $id; ?>&amp;size=s"></script><div class="clear"></div></div>
   ...
   function form($instance) {
        ...
        <p>
            <label for="<?php echo $this->get_field_id('id'); ?>"><?php _e('Flickr ID (<a href="http://www.idgettr.com" target="_blank">idGettr</a>):','nilmini'); ?></label>
...
/*-----------------------------------------------------------------------------------*/
 ...
 class nilmini_sociallinks extends WP_Widget {
	...
	function nilmini_sociallinks() {
		$widget_ops = array('description' => 'Link to your social profiles.' , 'nilmini');

		parent::WP_Widget(false, __('Social Links Widget', 'nilmini'),$widget_ops);
	}
        ...
        <ul class="sociallinks">
			...
			if($facebook != '' && $target != ''){
				...
				echo '<li><a href="'.$facebook.'" class="facebook" title="Facebook">Facebook</a></li>';
                        ...
                        ?>
			<?php if($diaspora != '' && $target != ''){
				echo '<li><a href="'.$diaspora.'" class="diaspora" title="Diaspora*" target="_blank">Diaspora*</a></li>';
			} elseif($diaspora != '') {
				echo '<li><a href="'.$diaspora.'" class="diaspora" title="Diaspora*">Diaspora*</a></li>';
			}
			?>
			<?php 
   ...
   function form($instance) { 
		...
		$diaspora = esc_attr($instance['diaspora']);
		...
		<p>
            <label for="<?php echo $this->get_field_id('diaspora'); ?>"><?php _e('Diaspora* URL:','nilmini'); ?></label>
            <input type="text" name="<?php echo $this->get_field_name('diaspora'); ?>" value="<?php echo $diaspora; ?>" class="widefat" id="<?php echo $this->get_field_id('diaspora'); ?>" />
        </p>
...
class nilmini_video extends WP_Widget {
	...
	function nilmini_video() {
		$widget_ops = array('description' => 'Show a custom featured video.' , 'nilmini');

		parent::WP_Widget(false, __('Video Widget', 'nilmini'),$widget_ops);
	}
